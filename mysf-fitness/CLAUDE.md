...
- **localStorage keys in use:** `mysf-workout-log`, `mysf-nutrition-log`, `mysf-session-log`, `mysf-bodyweight-log`, `mysf-program`, `mysf-nutrition-target`, `mysf-usda-api-key`, `mysf-onboarded`, `mysf-last-export`, `mysf-saved-meals`, `mysf-custom-foods`, `mysf-tdee-profile`, `mysf-builder-draft`. All are per-device only.
- **`mysf-barcode-cache` and `mysf-builder-draft` are the exceptions to the backup rule below.** `mysf-barcode-cache` is a disposable performance cache (barcode → last-looked-up product) that Open Food Facts can always regenerate. `mysf-builder-draft` holds whatever in-progress meal is currently being built (items added but not yet logged) purely so a reload/crash mid-build doesn't lose it — it's cleared the moment the meal is logged or explicitly canceled, and restoring someone's stale half-built meal onto a different device via backup wouldn't make sense. Neither is user-authored data worth preserving long-term, so both are deliberately excluded from `exportFullBackup()`/`performRestore()`.
